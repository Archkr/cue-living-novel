...
try {
  ...
  const parserSelect = settings.locator('select[name="parserConnectionId"]');
  const beforeParserDraft = (await fixture(page)).patches.length;
  await parserSelect.selectOption("text-1");
  assert.equal((await fixture(page)).patches.length, beforeParserDraft, "advanced connection waits for Apply");
  assert.match(await settings.locator("[data-status]").innerText(), /not applied/);
  await page.evaluate(() => (window as any).settingsFixture.panel.setConnectionCatalog("planner", { status: "loading", options: [] }));
  assert.equal(await parserSelect.inputValue(), "text-1", "catalog refresh preserves a draft even when its option is missing");
  ...
  assert.equal(await parserSelect.inputValue(), "text-1", "everyday saves and host echoes preserve the connection draft");
  ...
  assert.equal(applied?.parserConnectionId, "text-1", "Apply saves the advanced story reader connection");
  assert.equal(await settings.locator('select[name="setupParserConnectionId"]').inputValue(), "text-1", "setup reflects the applied connection");
  ...
  await settings.locator("[data-apply]").click();
  assert.equal((await lastPatch(page))?.parserConnectionId, null, "Lumiverse default clears the saved connection");
